...
from imloc.utils.metrics import cal_error_auc, check_data_hist, cal_reproj_dists_H
...
def eval_hpatches(
    ...
    lprint_(
        f">>Finished, pairs={len(match_time)} match_failed={match_failed} matches={np.mean(n_matches):.1f} match_time={np.mean(match_time):.6f}s"
    )
