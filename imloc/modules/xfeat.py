...
from third_party.xfeat.modules.xfeat import XFeat as XFeatModel
from .base import FeatureDetection, Matching
from imloc.utils.data_io import load_gray_scale_tensor_cv


class XFeat(FeatureDetection, Matching):
    def __init__(self, args=None):
        ...
        self.model = XFeatModel(weights=args['weights']).eval().to(self.device)
    ...
    def extract_features(self, gray):
        ...
        pred = self.model.detectAndCompute(gray)[0]
        kpts = pred['keypoints']
    ...
    def count_parameters(self):
        input = torch.rand(1, 1, 1024, 1024).to(self.device)
        
        flops, params = thop.profile(self.model, inputs=({"image":input},))
