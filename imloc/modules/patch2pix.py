...
import imloc
from imloc.utils.data_io import load_im_tensor
...
class Patch2Pix(Matching):
    ...
    def load_im(self, im_path):
        im, scale = load_im_flexible(
            im_path, 2, self.model.upsample, imsize=self.imsize
        )
...
class NCNet(Matching):
    ...
    def match_pairs(self, im1_path, im2_path):
        matches, scores, _ = estimate_matches(
            self.model,
            im1_path,
            im2_path,
            ksize=self.ksize,
            ncn_thres=self.match_threshold,
            eval_type="coarse",
            imsize=self.imsize,
        )
        kpts1 = matches[:, :2]
...
class Patch2PixRefined(Matching):
    ...
    def match_pairs(self, im1_path, im2_path):
        im1, grey1, sc1 = load_im_tensor(
            im1_path, self.device, imsize=self.imsize, with_gray=True
        )
        im2, grey2, sc2 = load_im_tensor(
            im2_path, self.device, imsize=self.imsize, with_gray=True
        )
        ...
        refined_matches, scores, _ = self.fmatcher.refine_matches(
            im1, im2, coarse_matches, io_thres=self.match_threshold
        )
